...
var placePointers = function(){
    var source = "<img id = \"source\" class = \"pointer-image\" src = \"../images/source.png\" draggable=true ondragstart='onDragStart(event);'>"
    var destination = "<img id = \"destination\" class = \"pointer-image\" src = \"../images/destination.png\" draggable=true ondragstart='onDragStart(event);'>"
    ...
    $(`#row_${sIndex[0]}`).children(`#col_${sIndex[1]}`).append(source);
    $(`#row_${dIndex[0]}`).children(`#col_${dIndex[1]}`).append(destination);
...
var ui = function(type){
    ...
    if (type=="shortest_path"){
        ...
        for(var j = 0;j<numRows;j++){
            ...
            $(`#grid_row_${j}`).append(`<div id='row_${j}' class='grid-row-div'></div>`);
            allNodes[j] = new Array(numCols);
                for(var i=0; i<numCols;i++){
                    $(`#row_${j}`).append(`<div id = 'col_${i}' class='grid-element' ondragenter='onDragEnter(event);' ondragleave='onDragLeave(event);' ondragover='onDragOver(event);' ondrop='onDrop(event);'></div>`);
...
function onDragStart(event) {
    if (currentSelection !="wall"){
        event
            .dataTransfer
            .setData('text/plain', event.target.id + "--" + $(event.target).parent('div').attr('id') + "--" + $(event.target).parent('div').parent('div').attr("id"));
